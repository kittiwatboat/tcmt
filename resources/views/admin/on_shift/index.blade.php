...
<html lang="en" class="light">
    ...
    <body class="py-5">
                ...
                <div class="top-bar">
                    ...
                    <nav aria-label="breadcrumb" class="-intro-x mr-auto hidden sm:flex">
                        <ol class="breadcrumb">
                            ...
                            <li class="breadcrumb-item active" aria-current="page">"Working time</li>
                        </ol>
                    </nav>
                    <!-- END: Breadcrumb -->
                ...
                <div class="grid grid-cols-12 gap-6 mt-5">
                    ...
                    <div class="intro-y col-span-12 overflow-auto lg:overflow-visible">
                        <table class="table table-report -mt-2">
                            <thead>
                                <tr>
                                    <th class="whitespace-nowrap">SHIFT TIME</th>
                            ...
                            <tbody>
                                @foreach ($on_shift as $osh)
                                <tr class="intro-x">
                                    <td >
                                        <div> {{$osh->shift_time}} </div>
                                    </td>
                                    <td >
                                        <div> {{$osh->start_work}} </div>
                                    </td>
                                    <td>
                                        <div> {{$osh->end_work}} </div>
                                    </td>
                                    <td class="table-report__action w-56">
                                        <div class="flex justify-center items-center">
                                            {{-- @if ($ul->role != 'super admin') --}}
                                            <a class="flex items-center mr-3" href="javascript:;" data-tw-toggle="modal" data-tw-target="#edit-modal-user{{$osh->id}}" ><i data-lucide="check-square" class="w-4 h-4 mr-1"></i>Edit</a>
                                            <div class="text-center"> <a href="javascript:;" data-tw-toggle="modal" data-tw-target="#delete-modal-user-{{$osh->id}}" class="btn btn-outline-danger"> Delete</a> </div> <!-- END: Modal Toggle -->
                                            {{-- @endif --}}
                                        </div>
                                    </td>
                                </tr>
                    ...
                    <div class="intro-y col-span-12 flex flex-wrap sm:flex-row sm:flex-nowrap items-center">
                        <nav class="w-full sm:w-auto sm:mr-auto">
                            <ul class="pagination">
                                {{ $on_shift->links('vendor.pagination.custom') }}
                            </ul>
                        </nav>
        ...
        <!-- BEGIN: Create Admin Modal -->
        @foreach ($on_shift as $osh2)
        <div id="edit-modal-user{{$osh2->id}}" class="modal" tabindex="-1" aria-hidden="true">
            <div class="modal-dialog modal-xl">
                <div class="modal-content">
                    <!-- BEGIN: Modal Header -->
                        <div class="modal-header">
                            <h2 class="font-medium text-base mr-auto">Edit Shift Time <b>{{$osh2->shift_time}}</b></h2>
                        </div>
                        <!-- END: Modal Header -->
                        <form action="{{url('/admin/update_user')}}" method="POST" enctype="multipart/form-data">
                            @csrf
                            <input type="hidden" name="id" value="{{$osh2->id}}">
                        <!-- BEGIN: Modal Body -->
                        <div class="modal-body grid grid-cols-12 gap-4 gap-y-3">
                            <div class="col-span-12 sm:col-span-6">
                                <label for="modal-form-1" class="form-label">Shift time</label>
                                <input id="modal-form-1" name="shift_time" type="text" class="form-control" value="{{$osh2->shift_time}}" required>
                            </div>
                            <div class="col-span-12 sm:col-span-6">
                                <label for="modal-form-2" class="form-label">Start work</label>
                                <input id="modal-form-2" name="start_work" type="date" class="form-control" value="{{$osh2->start_time}}">
                            </div>
                            <div class="col-span-12 sm:col-span-6">
                                <label for="modal-form-2" class="form-label">End work</label>
                                <input id="modal-form-2" name="end_work" type="date" class="form-control" value="{{$osh2->end_time}}">
                            </div>
